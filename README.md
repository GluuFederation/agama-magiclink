...
Below is a typical configuration for the Agama-MagicLink project:
...
| ----------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- |
| `hostName`        | Yes      | The hostname of the Identity Provider (IdP) server used when generating Magic Link URLs.                                                                                                   | `idp.example.com` |
| `secretKey`       | Yes      | Secret key used to sign and validate JWT tokens generated by the project. This value must remain consistent across application restarts so that previously issued tokens can be validated. | `your-secret-key` |
| `tokenExpiration` | No       | Magic Link token lifetime in minutes.                                                                                                                                                      | `10`              |

> **Note**
>
> The `secretKey` is used for signing JWTs. Although it is technically possible to generate a key dynamically, the signing key must remain consistent for token validation. Therefore, the key should be stored securely and reused rather than regenerated on every startup.
